...
class Model(object):
    ...
    def one_step(self, sess, x, y):
        ...
        return sess.run(outputs, feed_dict={self.x: x, self.y: y}, run_metadata=run_metadata, options=options)
    ...
    def episode_step(self, sess, x, y, clear_memory=False):
        ...
        for xx, yy in zip(x, y):
            out = sess.run(outputs, feed_dict={self.x: xx, self.y: yy}, run_metadata=run_metadata, options=options)
            profile(run_metadata, step)
            step = step + 1
            loss = out[0]
            losses.append(loss)
